...
import {
  ...
  DEFAULT_TARGET_HANDS,
  PLAYERS_PER_TEAM,
  dealerSeatsFromMembers,
...
function emptyMembers(teamCount) {
  return Array.from({ length: teamCount }, () => Array.from({ length: PLAYERS_PER_TEAM }, () => ''))
}
...
export default function StartPage() {
  ...
  const [members, setMembers] = useState(emptyMembers(2))
  ...
  const resetForm = () => {
    setGameType(null)
    setNames(['', ''])
    setMembers(emptyMembers(2))
    setWinMode('points')
    setTargetHands(DEFAULT_TARGET_HANDS)
  ...
  const handleSelectType = (type) => {
    setGameType(type)
    if (type === 'individual') {
      setNames(Array.from({ length: DEFAULT_INDIVIDUAL_PLAYERS }, () => ''))
    } else {
      setNames(['', ''])
    }
    setMembers(emptyMembers(2))
  ...
  const handleTeamCountChange = (teamCount) => {
    ...
    setMembers((prev) => emptyMembers(teamCount).map((row, i) => prev[i] || row))
    setFirstDealerIndex((i) => (i >= teamCount * PLAYERS_PER_TEAM ? 0 : i))
  }

  const handleIndividualCountChange = (delta) => {
    const nextLen = Math.min(MAX_INDIVIDUAL_PLAYERS, Math.max(MIN_INDIVIDUAL_PLAYERS, names.length + delta))
    setNames((prev) => Array.from({ length: nextLen }, (_, i) => prev[i] || ''))
    setFirstDealerIndex((i) => (i >= nextLen ? 0 : i))
  }
  ...
  const resolvedTeamNames = names.map((n, i) => n.trim() || (gameType === 'twohanded' ? `Player ${i + 1}` : `Team ${i + 1}`))
  const dealerSeats = gameType === 'team'
    ? dealerSeatsFromMembers(resolvedTeamNames, members)
    : names.map((n, i) => ({
  ...
  const gameOptions = () => ({
    ...
    targetHands,
    firstDealerIndex,
    members: gameType === 'team' ? members : gameType === 'twohanded' ? names.map((n) => [n]) : undefined,
    twoHanded: gameType === 'twohanded',
  })

  const handleStart = (e) => {
    e.preventDefault()
    if (gameType === 'team' || gameType === 'twohanded') {
      const fallback = gameType === 'twohanded' ? 'Player' : 'Team'
      const teamNames = names.map((n, i) => n.trim() || `${fallback} ${i + 1}`)
      const game = startNewTeamGame(teamNames, gameOptions())
      navigate(`/game/${game.id}`)
    } else {
      const playerNames = names.map((n, i) => n.trim() || `Player ${i + 1}`)
      const game = startNewIndividualGame(playerNames, gameOptions())
      navigate(`/game/${game.id}`)
    }
  }
          ...
          <div className="w-full max-w-sm space-y-3">
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-white/70">2 or 3 teams of two, named players</p>
              </div>
            </button>
            <button
              type="button"
              onClick={() => handleSelectType('twohanded')}
              className="w-full flex items-center gap-4 p-5 rounded-xl glass border border-white/10 hover:bg-white/10 transition-colors text-left"
            >
              <div className="w-12 h-12 rounded-lg glass flex items-center justify-center shrink-0">
              ...
              <div>
                <p className="text-lg font-bold text-white">Two Handed</p>
                <p className="text-sm text-white/70">1 vs 1 — just two names</p>
              </div>
            </button>
            <button
              type="button"
        ...
        <form onSubmit={handleStart} className="w-full max-w-sm space-y-6">
          ...
          {names.map((name, i) => (
            <div key={i} className="space-y-3">
              ...
              {gameType === 'team' && members[i] && (
                <div className="grid grid-cols-2 gap-2">
                  {members[i].map((playerName, p) => (
                    <div key={p}>
                      <label className="text-xs font-medium text-white/50 block mb-1">PLAYER {p + 1}</label>
                      <input
                        type="text"
                        placeholder="Name"
                        value={playerName}
                        onChange={(e) => handleMemberChange(i, p, e.target.value)}
